...
/*
The following WP Action loads specific widgets. These widgets are the widgets that are not included inside the p5parent
theme because they are too specific. We must load them manually here. The code is placed inside the "after_setup_theme"
action to make sure that the parent theme is loaded first. It ensures that widgets can be correctly initalized.
*/
add_action( 'after_setup_theme', 'p5child_load_widgets' );
function p5child_load_widgets() {
	require_once(dirname(__FILE__) . '/widgets/want_a_website.php');
}
